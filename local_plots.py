...
def main():
    ...
    # Pre-processing data
    top_comps = comp_df.groupby('Company')['Date received'].count().sort_values(ascending=False)[:10].index
    ...
    fig = px.histogram(top_comps_df, x='datetime', template='plotly_white', title='Complaint counts by date & company'
